...
public class TWId {
	...
	private TWId(String id) {
		this.id = id;
				...
				//TWId定義了返回類型是返回一個物件實例 所以不能省略
	public static TWId newTWId(String id) {
		if (isRight(id)) {
			return new TWId(id);
		} else {
			return null; // throws new Exception();
		}
	}
    ...
    public char whichArea() {
        return id.charAt(0);
    }

	// static可以提供給外部做使用，但static比較單純是方法，跟物件實體無關
